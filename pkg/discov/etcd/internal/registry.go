/*
 * @Author: yujiajie
 * @Date: 2025-01-15 16:42:08
 * @LastEditors: yujiajie
 * @LastEditTime: 2025-03-21 17:00:46
 * @FilePath: /manyo/pkg/discov/etcd/internal/registry.go
 * @Description:
 */
package internal
	...
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			if vals, ok := c.values[key]; ok {
				...
				c.values[key] = map[string]string{
		...
		default:
			logger.Error("Unknown event type: %v", ev.Type)
		}
	}
}
...
func (c *cluster) load(cli EtcdClient, key string) int64 {
	...
	for {
		...
		logger.Error("%s, key is %s", err.Error(), key)
...
func (c *cluster) newClient() (EtcdClient, error) {
	cli, err := DialClient(c.endpoints)
	if err != nil {
		return nil, err
	}
...
func (c *cluster) watch(cli EtcdClient, key string, rev int64) {
	for {
		...
		if rev != 0 && errors.Is(err, rpctypes.ErrCompacted) {
			logger.Error("etcd watch stream has been compacted, try to reload, rev %d", rev)
			rev = c.load(cli, key)
		}
		logger.Error("%v", err)
	}
